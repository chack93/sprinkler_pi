defmodule SprinklerPiUiWeb.ScheduleControllerTest do
  use SprinklerPiUiWeb.ConnCase

  alias SprinklerPiUi.Controls

  @create_attrs %{time: "some time"}
  @update_attrs %{time: "some updated time"}
  @invalid_attrs %{time: nil}

  def fixture(:schedule) do
    {:ok, schedule} = Controls.create_schedule(@create_attrs)
    schedule
  end
  ...
  describe "update schedule" do
    ...
    test "redirects when data is valid", %{conn: conn, schedule: schedule} do
      ...
      assert html_response(conn, 200) =~ "some updated time"
    end
